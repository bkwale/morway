 ...
 * Accountant approves an exception invoice — optionally overriding account codes.
 */
export async function POST(
  ...
  const { userId, accountCodeOverrides, notes } = body as {
    userId: string
    accountCodeOverrides?: Record<string, string> // lineItemId -> accountCode
    notes?: string
  }
  ...
  // Apply any account code overrides to line items
  ...
  // Ensure all line items have account codes
  ...
  if (!xeroContact?.contactID) {
    return NextResponse.json(
      { error: 'Could not find or create Xero contact' },
      { status: 500 }
    )
  }
  ...
  if (!xeroBill?.invoiceID) {
    return NextResponse.json(
      { error: 'Xero bill creation failed' },
      { status: 500 }
    )
  }
  ...
  const action = hasOverrides
    ? EXCEPTION_ACTION.EDITED_AND_APPROVED
    : EXCEPTION_ACTION.APPROVED
